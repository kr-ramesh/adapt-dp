...
#SBATCH --output=logs/vanilla_common_1024%j.out
#SBATCH --error=logs/vanilla_common_1024%j.err
...
for target_epsilon in 8 16 4; do #0.05 0.01; do
    ...
    python -m torch.distributed.launch --nproc_per_node=1 --master_port "$master_port" train.py --dataset_name "$DATASET" --path_to_dataset "/export/fs06/kramesh3/datasets/${DATASET}" \
                                        ...
                                        --lr "$LR" --epochs "$EPOCHS" --target_epsilon "$target_epsilon" --clipping 'flat' --max_grad_norm $max_grad_norm --lora_r $lora_r --lora_alpha $lora_alpha --sequence_len $SEQUENCE_LEN
done
